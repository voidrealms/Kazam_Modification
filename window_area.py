...
#       window_select.py\
#       
#       *** Modified /usr/lib/python3/dist_packages/window_area.py
#       *** Modified by Bryan Cairns <bcairns@voidrealms.com>
#       *** Saves a .kazam_area file in the home path that allows the app to save and load the selected area of the screen
#
...
class AreaWindow(GObject.GObject):
    ...
    def save_accepted(self):
        strAreaFile = os.path.join(os.environ['HOME'],'.kazam_area')
        print('startx = %d' % self.startx)
        print('endx = %d' % self.endx)
        print('starty = %d' % self.starty)
        print('endy = %d' % self.endy)
        print(strAreaFile)
        

        text_file = open(strAreaFile, "w")

        text_file.write('startx:' + str(self.startx) + '\r\n')
        text_file.write('endx:' + str(self.endx) + '\r\n')
        text_file.write('starty:' + str(self.starty) + '\r\n')
        text_file.write('endy:' + str(self.endy) + '\r\n')
    ...
    def load_area(self):
        ...
        area_file = Path(strAreaFile)

        if area_file.exists():
            f = open(strAreaFile, "r")
            lines = f.readlines()
            f.close()
            print(lines)
            self.startx = self.get_number(lines[0])
            self.endx = self.get_number(lines[1])
            self.starty = self.get_number(lines[2])
            self.endy = self.get_number(lines[3])
            self.width  = self.endx - self.startx
            self.height = self.endy - self.starty
            self.g_startx = self.startx
            self.g_starty = self.starty
            self.g_endx = self.endx
            self.g_endy = self.endy
